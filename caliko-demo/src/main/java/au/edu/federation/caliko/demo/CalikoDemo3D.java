...
public class CalikoDemo3D implements CalikoDemo
{
	...
	{
		try {
			this.demoStructure3d = CalikoDemoStructure3DFactory.makeDemoStructure3D(demoNumber);
			if( this.demoStructure3d == null) {
				throw new IllegalArgumentException("No such demo number: " + demoNumber);
			}
			this.demoStructure3d.setup();
			mStructure = this.demoStructure3d.getStructure();			
		} catch(Exception ex) {
